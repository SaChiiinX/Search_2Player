...
;;  Asmt. 2
;; ===================================== 
;;  For this assignment, each domain must define its own:
;;    MOVE struct
;;    STATE struct
;;    FETCH-LEGAL-MOVES method (that takes a STATE as input)
;;    DO-MOVE method (that takes a STATE and MOVE as inputs)
;;      and generates a list (new-state action-cost) as output.
;;    STATES-EQUAL? method (that takes two STATEs as inputs)
;;    STATE-IS-GOAL? method (that takes a STATE as input)
;; =====================================
...
(let* (;; Four EIGHTS-MOVE instances
  ...
  (mapcar #'(lambda (pair)
	      ...
	      ;;     EIGHTS-MOVE instances in MV-VECTOR are legal for the
	      ;;     position BPOSN.
	      (let ((bposn (first pair))
		    (mv-indices (second pair)))
		;; Set the hash-table entry for BPOSN to the list of
		;; EIGHTS-MOVE instances that are legal moves if the blank
		;; is at BPOSN.
	  ...
	  '(((0 0) (1 2))
	    ((0 1) (1 2 3))
	    ((0 2) (1 3)) ;; e.g., Blank at (0 2), so SOUTH, WEST are legal 
	    ((1 0) (0 1 2))
...
;;  FANCY-SHOW-EIGHTS-SOLN to display long sequences of EIGHTS states,
;;  10 to a row.
...
(defun fancy-show-eights-soln (goal-node)
  (let* ((nodes (xbuild-path goal-node))
	...
	(dolist (gr first-ten-grids)
	  (dotimes (col 3)
	    (format t "~s "
		    (if (zerop (aref gr row col)) '_ (aref gr row col))))
	  (format t "  "))
	(format t "~%"))
      (format t "~%")
