...
(defun print-xnode (xnode str depth)
  ...
  (let ((st (xnode-state xnode)))
    ...
    (format str "XNODE:  (action = ~A) (depth = ~A) (value = ~A) ~%"
	    (xnode-action xnode) (xnode-depth xnode) (xnode-value xnode))
    ;; Display the current state
    (format str "         STATE: ~A~%" st)))

;;  MAKE-ROOT-XNODE -- like MAKE-ROOT-NODE, but for XPROBLEM/XNODE structs
;; --------------------------------------------------------------------------
