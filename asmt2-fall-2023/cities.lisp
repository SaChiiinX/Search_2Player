;; =====================================
;;  CMPU-365, Fall 2023
;;  Implementation of CITIES domain
;;  Asmt. 2
;; ===================================== 
...
(let ((sld-hash (make-hash-table)))
  (mapcar #'(lambda (city-sld-pair)
	      (setf (gethash (first city-sld-pair) sld-hash)
		    (second city-sld-pair)))
	  '((arad 366)
	    (bucharest 0)
	    (craiova 160)
	    (dobreta 242)
  ...
  ;; CITIES-HEURISTIC
  ;; --------------------------------------------
  ;;  Fetch straight-line distance (SLD) to Bucharest from the current city
