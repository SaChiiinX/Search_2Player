...
;;; $Id: gp.lisp,v 1.32 1999/05/26 03:59:20 corin Exp $
...
(defparameter *GP-CAPABILITIES* 
    '(:strips :equality :typing :conditional-effects :disjunctive-preconditions 
      :universal-preconditions :existential-preconditions :adl))
...
(defun PLAN (problem &key (domain nil) (levels 10) (all-plans nil)
  ...
  (when (check-requirements *gp-capabilities* problem domain)
    ...
    (let ((plan-graph (initialize-plan-graph problem domain))
      ...
      ;; Begin main loop
      (let ((total-action-mutex-count 0)
	    (total-proposition-mutex-count 0)
	    (total-mutex-count 0)
	    )
      ...
      (loop for level from 1 upto levels 
		...
		(setq leveled-off level))
	      ...
	      ;; Update total mutex count variables.
	      (incf total-action-mutex-count new-action-mutex-count)
	      (incf total-proposition-mutex-count new-proposition-mutex-count)
	      )		   
	    ;; End of let*
	    
	    (setf total-mutex-count (+ total-action-mutex-count total-proposition-mutex-count))
      ...
      (format t "~%MUTEX TOTALS FOR ALL LEVELS:~%")
      
      (format t "~&  ~A action mutexes  ~A proposition mutexes~%" total-action-mutex-count total-proposition-mutex-count)
      (format t "~&  ~A total mutexes~%" total-mutex-count)
      
      (format t "~%  ~%")
      ) ;; End of let surrounding main loop.
...
(defun process-precs (preconditions	; The list of preconditions that we're processing.
  ...
  (if (null preconditions)
	...
	;; keyword here.
	  ...
	  (t (let ((plugged-pre (plug pre bindings)))
