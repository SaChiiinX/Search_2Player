...
;;; $Id: domains.lisp,v 1.11 1998/08/26 06:04:19 corin Exp $
...
(defun LOAD-domains (&rest domains)
  (dolist (domain domains)
    (let ((*package* (find-package "DOMAINS")))
      (load (merge-pathnames domain "*.pddl"))))
  t)
  ...
  (let ((domain nil)
    ...
    (when situation
      ...
      (setq init (append '(domains::and)
			 (combine-inits (situation-init situation) (cdr init))))
    ...
    (let ((situation 
	   (make-situation
	    :name name
	    :domain domain
	    :init (cdr init)		; Lose the leading AND in the problem's inits
	    :objects objects)))
      (setf (get name 'situation) situation)
      (pushnew name *situations*))
