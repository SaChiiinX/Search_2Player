...
;;           DEPTH, ignored
;;  OUTPUT:  None
;;  SIDE EFFECT:   Shows the contents of the given eights puzzle game in the 
...
(defun show-eights (game str depth)
  ...
  (let ((locs (eights-locations game)))
    (dotimes (row 3)
      ;; Provide a little bit of indenting at beginning of a row
      (format str "  ")
      (dotimes (col 3)
	(format str "~A " (let ((tile (aref locs row col)))
			  (if (= tile +blank-num+)
			      +blank-symbol+
			    tile))))
...
(defun eights-equal? (game1 game2)
  (equalp (eights-locations game1) 
	  (eights-locations game2)))
...
(defun eights-goal? (game)
  (equalp (eights-locations game) *eights-goal-array*))
...
(defun do-blank-move (game row-delta col-delta)
  (let* ((orig-blank-row (eights-blank-row game))
	 (orig-blank-col (eights-blank-col game))
	 (new-blank-row (+ orig-blank-row row-delta))
	 (new-blank-col (+ orig-blank-col col-delta)))
    (cond
     ;; Case 1:  Illegal move!
     ((on-board new-blank-row new-blank-col)
      (let ((new-locs (copy-array (eights-locations game))))
	;; Swap blank with tile at new blank position
	(setf (aref new-locs orig-blank-row orig-blank-col) 
	  (aref new-locs new-blank-row new-blank-col))
	(setf (aref new-locs new-blank-row new-blank-col)
	  +blank-num+)
	;; Return the new game
	(make-eights :locations new-locs
		     :blank-row new-blank-row
		     :blank-col new-blank-col)))
     ;; Case 2:  Illegal move!
     (t
      nil))))
